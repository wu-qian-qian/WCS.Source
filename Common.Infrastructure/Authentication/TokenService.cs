...
namespace Common.Infrastructure.Authentication
{
    public sealed class TokenService(IOptions<JWTOptions> options) : ITokenService
    {
        ...
        {
            TimeSpan ts = TimeSpan.FromSeconds(options.Value.ExpireSeconds);
            //构建密钥
            var securityKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(options.Value.Key));
            //加密算法进行密钥配置
            var credentials = new SigningCredentials(securityKey, SecurityAlgorithms.HmacSha256Signature);
            //构建安全JWT
            var tokenDescriptor = new JwtSecurityToken(options.Value.Issuer, options.Value.Audience, claims, expires: DateTime.Now.Add(ts), signingCredentials: credentials);
